...
- **Automated Flight Logbook**: Automatically tracks flight sessions from engine start to shutdown (out, off, on, in timestamps). Calculates block time, flight time, distance flown, max altitude, and ground speed, and stores GPS route breadcrumbs for in-app map replay. Supports search, filtering, aggregate pilot statistics, and CSV/JSON export.
- **Landing Rate & Touchdown Analytics**: Captures vertical speed (FPM), normal G-force, pitch, bank, and groundspeed at the exact moment of main gear touchdown. Evaluates landing quality (Butter Smooth, Soft, Normal, Firm, Hard) and tracks career landing averages.
